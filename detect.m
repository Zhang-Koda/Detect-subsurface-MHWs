...
m90_three = cat(3,m90,m90,m90);
for i = 1:size(m90_three,1)
    for j = 1:size(m90_three,2)
        m90long(i,j,:) = smooth(m90_three(i,j,:),vsmoothPercentileWidth,'moving');
    end
end
%m90long=smoothdata(cat(3,m90,m90,m90),3,'movmean',vsmoothPercentileWidth);
%m90long(i,j,:)=smooth(m90_three(i,j,:),vsmoothPercentileWidth,'moving');
m90=m90long(:,:,367:367+365);
mclim_three = cat(3,mclim,mclim,mclim);
for i = 1:size(mclim_three,1)
    for j = 1:size(mclim_three,2)
        mclimlong(i,j,:) = smooth(mclim_three(i,j,:),vsmoothPercentileWidth,'moving');
    end
end
